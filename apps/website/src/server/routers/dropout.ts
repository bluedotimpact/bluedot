...
export const dropoutRouter = router({
  ...
  dropoutOrDeferral: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // validate the deferral target against the current round's course (falling back to the
      // registration's course link when there is no round), rather than trusting the client's
      // round choice.
      if (type === 'Deferral' && newRoundId) {
        ...
        // `||` (not `??`): an empty Airtable link syncs to '' rather than null.
