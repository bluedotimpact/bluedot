...
describe('courseRegistrations.getByCourseId', () => {
  ...
  test('does not return registrations with a non-Accept decision', async () => {
    await testDb.insert(courseRegistrationTable, {
      id: 'reg1', email: 'test@example.com', userId: 'user-test', courseId: otherCourseId, decision: 'Reject',
    });
    await testDb.insert(courseRegistrationTable, {
      id: 'reg2', email: 'test@example.com', userId: 'user-test', courseId: otherCourseId, decision: 'Withdrawn',
    });
...
describe('courseRegistrations.getAll', () => {
  ...
  test('returns all non-Withdrawn registrations for the auth email, including null-decision rows', async () => {
    await testDb.insert(courseRegistrationTable, {
      id: 'reg-accept', email: 'test@example.com', userId: 'user-test', courseId: 'a', decision: 'Accept',
    });
    await testDb.insert(courseRegistrationTable, {
      id: 'reg-pending', email: 'test@example.com', userId: 'user-test', courseId: 'b', decision: null,
    });
    await testDb.insert(courseRegistrationTable, {
      id: 'reg-withdrawn', email: 'test@example.com', userId: 'user-test', courseId: 'c', decision: 'Withdrawn',
    });
    await testDb.insert(courseRegistrationTable, {
