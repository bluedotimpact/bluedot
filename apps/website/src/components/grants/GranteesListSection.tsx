...
type PublicGrantGrantee = inferRouterOutputs<AppRouter>['grants']['getAllPublicGrantees'][number];
...
const GranteeListItem = ({ grantee }: { grantee: PublicGrantGrantee }) => {
  const amount = formatAmountUsd(grantee.amountUsd);
  const subtitle = amount ? `${grantee.name} • ${amount}` : grantee.name;
  const Container = grantee.profileOrProjectUrl ? 'a' : 'div';

  return (
    <Container
      {...(grantee.profileOrProjectUrl
        ? {
          href: grantee.profileOrProjectUrl,
          target: '_blank',
      ...
      className={clsx(
        ...
        grantee.profileOrProjectUrl && 'transition-colors hover:border-bluedot-navy/18 hover:bg-white/90',
      )}
    >
          ...
          {!!visibleGrantees?.length && (
            <ul className="list-none flex flex-col gap-4 min-[680px]:gap-5">
              {visibleGrantees.map((grantee) => (
                <li key={`${grantee.name}-${grantee.projectName}`}>
