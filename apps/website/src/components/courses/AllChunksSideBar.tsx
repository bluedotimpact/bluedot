...
type AllChunksSideBarCollapsibleProps = {
  unitWithChunks: UnitWithChunks;
...
const AllChunksSideBarCollapsible: React.FC<AllChunksSideBarCollapsibleProps> = ({
  unitWithChunks,
  isCurrentUnit,
  currentChunkIndex,
  onChunkSelect,
  ...
  return (
    <div className="relative">
      ...
      <details
        open // Always open in "View All" mode
      ...
      >
        <summary className="flex flex-row items-center mx-[24px] px-[24px] md:px-[12px] py-[15px] gap-[8px] text-left cursor-pointer hover:bg-[rgba(42,45,52,0.05)] hover:rounded-[10px] transition-colors">
          ...
          >
            {unitWithChunks.unitNumber}. {unitWithChunks.unit.title}
          </p>
        ...
        <div className="flex flex-col items-start px-0 pb-[16px] gap-[4px]">
          {unitWithChunks.chunks.map((chunk, index) => {
            const isActive = isCurrentUnit && currentChunkIndex === index;
            return (
              <button
                type="button"
                key={chunk.id}
                onClick={() => onChunkSelect(unitWithChunks.unitNumber, index)}
                className={clsx(
...
const AllChunksSideBar: React.FC<AllChunksSideBarProps> = ({
  courseTitle,
  className,
  allChunks,
  currentUnitNumber,
  ...
  const isCurrentUnit = (unitNumber: number): boolean => {
    return currentUnitNumber === unitNumber;
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto">
        {allChunks.map((unitWithChunks) => (
          <AllChunksSideBarCollapsible
            key={unitWithChunks.unit.id}
            unitWithChunks={unitWithChunks}
            isCurrentUnit={isCurrentUnit(unitWithChunks.unitNumber)}
...
export default AllChunksSideBar;
