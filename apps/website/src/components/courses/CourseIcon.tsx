...
/**
 * Always decorative: every caller renders the course title alongside it, so the
 * whole tile (including the badge letter) is hidden from assistive technology.
 */
export const CourseIcon: React.FC<CourseIconProps> = ({
  ...
  return (
    <div
      ...
      className={clsx(
        config.container,
        'relative flex shrink-0 items-center justify-center rounded-surface bg-accent',
        className,
      )}
    ...
    >
      {iconSrc ? (
        ...
        <FallbackGlyph className={config.image} />
      )}
      {badge && (
        <span
          className={clsx(
            config.badge,
            'absolute -top-0.5 -right-0.5 flex items-center justify-center rounded-full bg-raised font-bold text-primary ring-1 ring-subtle',
          )}
        >
          {badge}
        </span>
