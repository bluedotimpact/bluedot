...
const assignTab = (row: CourseListRowProps): CourseTab | null => {
  const {
    courseRegistration: cr, isDroppedOut, isDeferred, isDeferredToAnotherRound,
  } = row;

  if (isDroppedOut && !isDeferred) return 'pastCourses';
  if (cr.roundStatus === 'Active') return 'inProgress';
  if (cr.roundStatus === 'Future') return 'upcoming';
  // The round has ended, and a duplicate registration carries the course they moved to.
  if (isDeferredToAnotherRound && !cr.certificateCreatedAt) return null;
  return 'pastCourses';
};
