import {
  H1, H3, H4, P, ProgressDots, Section,
} from '@bluedot/ui';
...
// Only show courses that have visual config
const HOMEPAGE_COURSE_SLUGS = Object.keys(COURSE_VISUAL_CONFIG);
...
const CourseCardsGrid = ({
  ...
  <div className="hidden lg:flex flex-col items-center gap-8 w-full max-w-screen-xl mx-auto">
    ...
    <P className="text-size-xs font-medium tracking-[1.5px] text-bluedot-navy/60">
    ...
    {/* "Deep-dives" heading */}
    <P className="text-size-xs font-medium uppercase tracking-[1.5px] text-bluedot-navy/60">
      Deep-dives
    </P>
      ...
      <div className="relative z-10 flex flex-col h-full min-h-[380px] p-6 md:p-8 lg:p-10">
        ...
        <div className="flex flex-col gap-6">
          <div className="flex flex-col gap-4">
            ...
            <P className="text-size-sm leading-[1.55] tracking-[-0.002em] text-white">
...
const CourseSection = () => {
  ...
  const courses = useMemo((): Course[] => {
    ...
    return HOMEPAGE_COURSE_SLUGS
      .flatMap((slug): Course[] => {
        const dbCourse = dbCourses.find((c) => c.slug === slug);
        const config = COURSE_VISUAL_CONFIG[slug]!;
        if (!dbCourse) return [];
