  ...
  return (
    <section className="w-full bg-[#FAFAFA]">
      <div className="max-w-max-width mx-auto px-5 py-10 bd-md:px-8 bd-md:py-12 lg:px-spacing-x">
        <H2 className="text-[18px] bd-md:text-[20px] font-medium leading-[140%] text-bluedot-navy/60 text-center mb-8 tracking-[-0.01em]">
          {title}
        </H2>
          ...
          {organizations.map((org) => {
            const LogoImage = (
              <img
                src={org.logo}
                alt={org.name}
                className="h-8 bd-md:h-10 w-auto object-contain opacity-70 hover:opacity-100 transition-opacity duration-200"
              />
            );
