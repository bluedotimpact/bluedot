    ...
    <>
      ...
      <div className="mx-auto max-w-6xl px-6 py-10 sm:px-10 sm:py-14">
        ...
        <div className="mt-9 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
          {apps.map((app) => (
            <Link key={app.id} href={app.href} className="group flex min-w-0 flex-col rounded-overlay border border-subtle bg-raised p-6 transition-colors hover:border-strong hover:bg-canvas focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-focus">
              ...
              <span className="mt-7 flex items-center justify-between text-size-xs font-medium text-accent">Open app<PortalIcon name="arrow" className="transition-transform group-hover:translate-x-1" /></span>
            </Link>
          ))}
        </div>
      </div>
