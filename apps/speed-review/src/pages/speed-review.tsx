...
const SpeedReviewPage = () => {
  ...
  const promptOpen = useNavigationState((navigation) => navigation.promptOpen);
  const setSessionActive = useNavigationState((navigation) => navigation.setSessionActive);
  const failedRating = useRef<RatingValue | null>(null);
  useEffect(() => {
  ...
  // Keyboard shortcuts
  useEffect(() => {
    if (state.status !== 'reviewing' || saveError) return;
    const onKey = (e: KeyboardEvent) => {
      const target = e.target as HTMLElement;
      if (e.metaKey || e.ctrlKey || e.altKey || target.isContentEditable || document.querySelector('[role=dialog]') !== null || useNavigationState.getState().pendingWrites > 0) return;
  ...
  return (
    <div className="min-h-[calc(100dvh-4rem)] bg-canvas py-4 sm:py-8 px-3 sm:px-4">
      <div className="max-w-3xl mx-auto space-y-4">
        <div className="bg-raised rounded-xl border border-subtle px-5 py-3 flex items-center gap-4">
          <div className="flex-1">
            <CountdownTimer
              ...
              paused={state.timerPaused || pendingWrites > 0 || promptOpen}
          ...
          <button
            ...
            className="flex size-11 items-center justify-center rounded-surface text-secondary hover:bg-tint transition-colors shrink-0 disabled:opacity-40"
        ...
        <div className="bg-raised rounded-xl border border-subtle p-4 sm:p-6">
          <ApplicationCard
            key={current.id}
            application={current}
      ...
      {milestoneToast && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 bg-accent text-white text-size-sm px-5 py-2.5 rounded-full shadow-lg pointer-events-none max-w-[90vw] text-center">
          {milestoneToast}
        </div>
      )}
