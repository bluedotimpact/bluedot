...
try {
  ...
  assert.equal(await page.getByRole('button', { name: /^Biosecurity/ }).isDisabled(), true);
  ...
  if (output) {
    for (const [name, device] of [
      ...
      await shot.getByRole('button', { name: 'Show names (n)', exact: true }).click();
  ...
  console.log(`PASS Scout sign-in, confirmations, failed save/retry, pending-write guards, skip/navigation, and ${sizes.length} viewports`);
} finally {
  await browser.close();
}
