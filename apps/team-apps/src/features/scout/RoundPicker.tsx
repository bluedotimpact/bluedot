import type { InvitedThisWeek, QueueItem } from './types';
import {
  courses, roundKey, roundLabel, roundsFor,
} from './reviewQueue';
import { panel } from './reviewStyles';

const invitedLabel = (entry?: { total: number; viaApp: number }) => {
  if (!entry || entry.total === 0) return 'none invited this week';
  return `${entry.total} invited this week${entry.viaApp > 0 ? ` (${entry.viaApp} via the app)` : ''}`;
};

export const RoundPicker = ({ items, invited, onSelect }: { items: QueueItem[]; invited: InvitedThisWeek; onSelect: (item: QueueItem) => void }) => (
  <section aria-label="Choose a round" className={`${panel} mx-auto w-full max-w-3xl space-y-5 p-5 sm:p-8`}>
    ...
    <div className="space-y-4">{courses.map((course) => <details key={course} open><summary className="min-h-11 cursor-pointer py-3 text-size-sm font-semibold text-secondary">{course} <span className="font-normal">· {items.filter((item) => item.course === course).length} to review · {invitedLabel(invited[course])}</span></summary><div className="space-y-2">{roundsFor(items, course).map((item) => <button key={roundKey(item)} type="button" data-testid={`choose-round-${roundKey(item)}`} onClick={() => onSelect(item)} className="flex min-h-14 w-full items-center justify-between gap-3 rounded-surface border border-subtle px-4 py-3 text-left text-size-sm font-medium hover:border-accent hover:bg-tint"><span>{roundLabel(item)}</span><span className="shrink-0 text-size-xs text-secondary">{items.filter((p) => roundKey(p) === roundKey(item)).length} to review →</span></button>)}{roundsFor(items, course).length === 0 && <p className="py-2 text-size-xs text-secondary">No rounds from this course in the current Scout queue.</p>}</div></details>)}</div>
  </section>
);
