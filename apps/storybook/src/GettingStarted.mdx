...
`H1` and `H2` are no longer identical: `H1` is `text-size-2xl`, `H2` is `text-size-xl`; both are semibold navy in Inter Display. `HeroSection`'s `<HeroH1>` is the on-dark marketing variant (white, regular weight, larger display ramp).
...
- **Weight** — the four static Inter cuts: `font-normal` (400), `font-medium` (500), `font-semibold` (600), `font-bold` (700). There is no 450/650 cut; arbitrary in-between weights snap to the nearest of these.
