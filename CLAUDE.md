...
- Colour: pick the semantic role token first (`text-primary`, `text-secondary`, `bg-canvas`, `bg-raised`, `border-default`, `bg-accent`, `text-error-fg`, `ring-focus`). Names mirror the Figma variables. Fall back to the raw `bluedot-*` palette (`text-bluedot-navy/60`) only when no role fits. Avoid raw hex (`text-[#0037ff]`) and Tailwind's default palette (`text-red-600`, `border-gray-300`). Type uses the size tokens (`text-size-md`). Exact pixel values like `gap-[3px]` are sometimes ok. Tokens are documented in `apps/storybook/src/GettingStarted.mdx` and defined in `libraries/ui/src/default-config/tailwind.css`.
