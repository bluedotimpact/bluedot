...
export type CourseCardProps = React.PropsWithChildren<{
  ...
  title: string,
  description: string,
  courseType: 'Crash course' | 'Self-paced' | 'In-depth course',
  cardType?: 'Featured' | 'Regular',
  image?: string
}>;

const courseLength = (courseType: 'Crash course' | 'Self-paced' | 'In-depth course') => {
  switch(courseType) {
    case 'Crash course':
      return '5 days';
    case 'Self-paced':
...
export const CourseCard: React.FC<CourseCardProps> = ({ children, className, title, description, courseType = 'Crash course', cardType = 'Regular', image }) => {
  return (
    cardType === 'Featured' ?
